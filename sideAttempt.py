...
class Car(object):
    ...
    def drawCar(self, canvas):
        ...
        canvas.create_image(self.CenterX,
                            self.centerY,image=self.carImage)

    def moveCar(self, data, moves):
        #moving the car side to side
        if((data.simpleLeftCurb+self.carWidth) < (self.CenterX)):
            if(moves == "Left"):
                self.CenterX -= 10
        if((self.CenterX+(self.carWidth)) < (data.simpleRightCurb)):
            if(moves == "Right"):
                self.CenterX += 10
...
def init(data):
    ...
    data.carCenterY = ((data.height/10)*9)+18
...
def rectangleCollision(TLx1, TLy1, w1, h1, TLx2, TLy2, w2, h2):
    #check if two rectangles have collided
    if((TLx1 <= TLx2) and (TLx2 <= (TLx1+w1))):     #left edge of box 2 is "in" box 1
        if((TLy1 <= TLy2) and (TLy2 <= (TLy1+h1))):
            return True #the top left corner of box 2 is inside box 1
        elif((TLy1 <= (TLy2+h2)) and ((TLy2+h2) <= (TLy1+h1))):
            return True #the bottom left corner of box 2 is inside box 1
    elif((TLx1 <= (TLx2+w2)) and ((TLx2+w2) <= (TLx1+w1))):  #right edge of box2 is "in" box 1
        if((TLy1 <= TLy2) and (TLy2 <= (TLy1+h1))):
            return True #the top right corner of box 2 is inside box 1
        elif((TLy1 <= (TLy2+h2)) and ((TLy2+h2) <= (TLy1+h1))):
            return True #the bottom right corner of box 2 is inside box 1
    else:
        return False

def checkCollisions(data, runningCactus, car):
    #check for collisions between the running cactus and car
    topLeftxCar = car.CenterX - (car.carWidth/2)
    topLeftyCar = car.centerY - (car.carHeight/2)
    topLeftxCactus = runningCactus.x - (runningCactus.width/2)
    topLeftyCactus = runningCactus.y - (runningCactus.height/2)
    collisionStatus = rectangleCollision(topLeftxCar, topLeftyCar, car.carWidth, car.carHeight,
                       topLeftxCactus, topLeftyCactus, runningCactus.width, runningCactus.height)
    if(collisionStatus):
...
def keyPressed(event, data):
    if(data.stillAlive):
        if (event.keysym == "Left"): 
            data.thisCar.moveCar(data, "Left")
        if(event.keysym == "Right"):
            data.thisCar.moveCar(data, "Right")
    if(event.keysym == "r"):
        init(data)
    #pass

def timerFired(data):
    checkCollisions(data, data.obstacle1, data.thisCar)
    if(data.stillAlive):
